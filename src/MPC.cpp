...
//init values for the vars & actuators
size_t x_start = 0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      //fitting for a 3rd degree polynomial. 1st degree can only fit a straight line. 3rd degree is needed to adjust for the curvature of the road
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double x = state[0]; //x-axis value
  double y = state[1]; //y-axis value
  double psi = state[2]; //psi angle
  double v = state[3]; //velocity
  double cte = state[4]; //cross track error from sim
  double epsi = state[5]; //rate of change epsilon
  ...
  for(size_t i = 0; i < delta_start; i++){
    vars_lowerbound[i] = std::numeric_limits<long int>::min();
    vars_upperbound[i] = std::numeric_limits<long int>::max();
  }
